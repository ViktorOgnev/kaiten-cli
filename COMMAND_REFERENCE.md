...
- Derivation only reproduces the real UUID on an on-premises installation; elsewhere Kaiten stores a random one. When a derived UUID finds no data row the command reads the card, whose response lists the addons available to it across every space of its board - the same set the server checks when authorizing a write - and retries with the registered UUID. A card that reports no such addon is a real answer: it can have no attachments.
...
- The lookup costs one extra read per card - the card itself - and falls back to the space listing only for responses that do not carry the addon data. Nothing is amortized across cards, so in a loop resolve the UUID once with space-addons.list and pass --addon-uid.
