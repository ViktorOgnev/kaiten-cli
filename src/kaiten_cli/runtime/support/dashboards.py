...
def validate_dashboard_compute_payload(tool, payload: dict[str, Any]) -> None:
    ...
    if not isinstance(widget_ids, list) or not widget_ids:
        raise ValidationError(tr("Field widget_ids must be a non-empty array."))
    if len(widget_ids) > 100:
        raise ValidationError(tr("Field widget_ids must contain at most 100 items."))
...
async def execute_dashboard_widgets_list(
    ...
    if reporter:
        reporter(tr("execution: synthetic widget list from dashboards.get?include=widgets"))
