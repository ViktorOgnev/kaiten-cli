...
У карточки, доска которой вообще не имеет аддонов, сервер этих данных не вкладывает; тогда
CLI обходит пространства доски через `GET /spaces`. То есть самый частый случай «аддон не
установлен» как раз и приводит к обходу всего листинга пространств тенанта – это норма, а
не исключение, и на большом тенанте это сотни пространств в одном ответе. Полностью
удавшийся обход – ответ: `list` отдаст `[]`, а `attach` объяснит, что писать некуда. Если
хотя бы одно чтение не удалось, ответа нет – команда попросит `--addon-uid`. Если поиск вокруг карточки завершиться не смог – не прочиталось пространство, листинг или
регистрация аддона, – `list` не отвечает пустым списком, а падает с ошибкой: пустой ответ
здесь неотличим от ответа про другой аддон.
