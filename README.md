...
ошибкой: пустой ответ здесь неотличим от ответа про другой аддон. Запись в такой ситуации
продолжается – неверный UUID сервер отклонит сам. Чтения для резолва кэшируются как
обычные справочные, поэтому цикл по карточкам платит за них один раз; `--addon-uid`
убирает их совсем.
