...
не нашлось строки данных – читают саму карточку и ищут регистрацию аддона в
`board.spaces[].addons`. С найденным UUID команды перечитывают данные аддона.

Список в карточке зависит от права `card.read` в каждом пространстве, а для чтения
данных аддона достаточно доступа к карточке через любое пространство её доски.
Поэтому отсутствие нужного аддона в этом списке, в том числе отсутствие самого
`board.spaces`, не доказывает отсутствие доступных вложений. В таком случае `list`
завершается ошибкой и просит `--addon-uid`, а запись отклоняется до PATCH. Так же
команды поступают, если карточку прочитать не удалось или найденный id не подходит
для маршрута API. С явно заданным UUID или найденной регистрацией пустой ответ
хранилища по-прежнему возвращается как `[]`.

Реконструировать состав пространств доски через `GET /spaces` CLI не пытается:
листинг фильтруется правом на чтение пространства и не доказывает, что видны все
пространства доски. Для записи также нужны права на обновление: найденная
регистрация сама по себе не гарантирует, что сервер разрешит PATCH.
...
Поиск регистрации стоит одного дополнительного чтения карточки; список пространств
не запрашивается. После нахождения другого UUID отдельно перечитываются данные аддона.
Между карточками поиск не переиспользуется, поэтому в цикле получите UUID один раз
через `space-addons list` и передавайте `--addon-uid`.
