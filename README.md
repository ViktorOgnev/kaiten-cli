...
```bash
uv tool install "git+https://github.com/ViktorOgnev/kaiten-cli.git@v0.2.0"
```
...
не нашлось строки данных – читают саму карточку: её ответ содержит аддоны, доступные ей
по всем пространствам доски (тот же набор, по которому сервер авторизует запись), и
перечитывают с найденным UUID. Если карточка сообщает, что такого аддона у неё нет, это
настоящий ответ – вложений у неё быть не может.

У карточки, доска которой вообще не имеет аддонов, сервер этих данных не вкладывает – и это
тоже ответ: доступных аддонов нет, `list` отдаст `[]`, а `attach` объяснит, что писать
некуда. Реконструировать состав пространств доски через `GET /spaces` CLI не пытается:
листинг фильтруется правом на чтение пространства, поэтому он не доказывает ни того, что
видны все пространства доски, ни того, что найденный в них кандидат единственный.

Если карточку прочитать не удалось, ответа нет – `list` попросит `--addon-uid`, а запись
будет отклонена, не дойдя до PATCH: сервер принимает запись в любой доступный карточке
аддон, поэтому неверная догадка не отскочит, а попадёт в чужие данные. Если карточку прочитать не удалось или в её ответе аддон нашёлся, но с id вне контракта
маршрута, `list` не отвечает пустым списком, а падает с ошибкой: пустой ответ здесь
неотличим от ответа про другой аддон.

Путь монтирования – не идентичность: два аддона могут отдаваться с разных хостов по
одному и тому же пути. Если под путём нашлось несколько разных аддонов, команда не
выбирает наугад, а требует `--addon-uid` – запись не в тот аддон положила бы вложения
GitHub в чужие данные.

Резолв стоит одного лишнего чтения на карточку; список пространств запрашивается только
для ответов, в которых этих данных нет. Между карточками это не амортизируется, поэтому в
цикле получите UUID один раз через `space-addons list` и передавайте `--addon-uid`.

Запись отказывается перезаписывать ключ, если сохранённое значение – не список объектов:
перезапись затирает ключ целиком, и всё непонятное CLI было бы потеряно. Такой ключ видно
через `card-addon-data get`, починить можно `card-addon-data set`.
...
`v0.2.0` — граница совместимости для перехода на новый Public API. Относительно `v0.1.28` сохранены все существующие имена и псевдонимы команд: добавлено 28 команд, удалённых команд нет. При этом намеренно изменены контракты получения списков, поэтому автоматизации следует проверить по правилам миграции ниже.
...
Агрегированные `cards.list-all`, `comments.batch-list`, `card-children.batch-list`, `time-logs.batch-list`, `company-users.list-all` и tree-команды сами обходят страницы. Они совместимы как с новым API, так и с legacy-маршрутами, которые пока игнорируют `limit`/`offset`: oversized первая страница принимается как полный старый ответ, а идентичная вторая страница не добавляется повторно. Legacy-режим виден в `--verbose` и в `stats.pagination_compatibility` trace-файла.

После уже полученного прогресса повтор страницы не считается legacy-режимом. Повтор, неизвестная форма ответа и исчерпание защитного `max_pages` завершаются ошибкой вместо возврата дублированного или незаметно обрезанного результата.
...
- `/spaces`, `/documents` и `/document-groups` читаются внутренней пагинацией по `limit=100` с `offset=0,100,200...` до первой короткой страницы; legacy-ответ без пагинации распознаётся по oversized первой или идентичной второй странице.
