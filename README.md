...
CLI обходит пространства доски через `GET /spaces`. Полностью удавшийся обход – тоже
ответ: `list` отдаст `[]`, а `attach` объяснит, что писать некуда. Если хотя бы одно
чтение не удалось, ответа нет – команда попросит `--addon-uid`. Если аддон не
нашёлся ни в одном доступном пространстве, `list` не отвечает пустым списком, а падает с
ошибкой: пустой ответ здесь неотличим от ответа про другой аддон. Это же произойдёт, если
аддон просто не установлен – тогда вложений у карточки действительно нет. Запись в такой
ситуации продолжается: неверный UUID сервер отклонит сам.
