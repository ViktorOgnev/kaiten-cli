...
не нашлось строки данных – выясняют, какой аддон на самом деле есть у доски карточки:
сначала её собственное пространство, потом остальные пространства этой доски (Kaiten
считает доступность аддона по всем ним), и перечитывают с найденным UUID. Если аддон не
...
Резолв стоит двух лишних чтений на карточку, а список пространств запрашивается только
если в собственном пространстве карточки аддона не оказалось. Ничего из этого не
амортизируется между карточками (чтение карточки персонально, а запись сбрасывает кэш
профиля), поэтому в цикле получите UUID один раз через `space-addons list` и передавайте
`--addon-uid`.
